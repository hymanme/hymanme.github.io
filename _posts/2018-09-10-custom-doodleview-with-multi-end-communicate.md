...
* 将`java`数据包装包成可传输的`json`字符串的工作也是交给 TransactionCenter 来处理，由 TransactionManager 处理也行，但是为了更加高效，我们让 TransactionManager 处理单个数据包，TransactionCenter 处理一次发送任务数据包集合，如果每次都装包单个数据包成为`json`字符串，这未免也太低效了。
